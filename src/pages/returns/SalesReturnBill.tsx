...
export default function SalesReturnBill() {
  ...
  const defaultBillNo = baseType === 'challan' ? 'CRNC0001' : 'CRN0001';
  const billNoKey = baseType === 'challan' ? 'lastSalesReturnChallanNo' : 'lastSalesReturnBillNo';
  ...
  const handleBillNoKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' || e.key === 'Tab') {
      const savedBills = JSON.parse(localStorage.getItem('savedSalesReturnBills') || '[]')
  ...
  const confirmSaveBill = async () => {
    ...
    const payload: InvoiceCreatePayload = {
      invoice_type: `sales-return-${baseType}`,
    ...
    setShowSaveModal(false)
    setSaveSuccessMessage(`Sales Return Bill [${billNo}] Saved Successfully! Total: ₹${invoiceValue.toFixed(2)}`)
  ...
  if (type.startsWith('modify') && !selectedModifyBill) {
    return <SalesReturnList onSelectBill={setSelectedModifyBill} type={type} />
  }
