...
export default function BrkReceiveBill() {
  ...
  const defaultBillNo = baseType === 'challan' ? 'BKRC0001' : 'BKR0001';
  const billNoKey = baseType === 'challan' ? 'lastBrkReceiveChallanNo' : 'lastBrkReceiveBillNo';
  ...
  const handleBillNoKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' || e.key === 'Tab') {
      const savedBills = JSON.parse(localStorage.getItem('savedBrkReceiveBills') || '[]')
  ...
  const confirmSaveBill = async () => {
    ...
    const payload: InvoiceCreatePayload = {
      invoice_type: `brk-receive-${baseType}`,
    ...
    setShowSaveModal(false)
    setSaveSuccessMessage(`Brk/Exp Receive Bill [${billNo}] Saved Successfully! Total: ₹${invoiceValue.toFixed(2)}`)
  ...
  if (type.startsWith('modify') && !selectedModifyBill) {
    return <BrkReceiveList onSelectBill={setSelectedModifyBill} type={type} />
  }
