...
import BrkIssueList from './BrkIssueList'
...
export default function BrkIssueBill() {
      ...
      const fetchBill = async () => {
        try {
          ...
          if (billInfo) {
            ...
            setPartyInvNo(billInfo.party_inv_no || '')
            if (billInfo.date) {
              ...
              setInvDateStr(`${d.getDate().toString().padStart(2, '0')}-${(d.getMonth()+1).toString().padStart(2, '0')}-${d.getFullYear().toString().slice(2)}`)
            }
  ...
  const defaultBillNo = baseType === 'challan' ? 'BKIC0001' : 'BKI0001';
  const billNoKey = baseType === 'challan' ? 'lastBrkIssueChallanNo' : 'lastBrkIssueBillNo';
  ...
  const handleBillNoKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' || e.key === 'Tab') {
      const savedBills = JSON.parse(localStorage.getItem('savedBrkIssueBills') || '[]')
  ...
  const confirmSaveBill = async () => {
    ...
    const payload: InvoiceCreatePayload = {
      invoice_type: `brk-issue-${baseType}`,
    ...
    setShowSaveModal(false)
    setSaveSuccessMessage(`Brk/Exp Issue Bill [${billNo}] Saved Successfully! Total: ₹${invoiceValue.toFixed(2)}`)
  ...
  if (type.startsWith('modify') && !selectedModifyBill) {
    return <BrkIssueList onSelectBill={setSelectedModifyBill} type={type} />
  }
