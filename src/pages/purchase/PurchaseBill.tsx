...
const parseExpiryDate = (input: string): string => {
  if (!input) return ''
  const digits = input.replace(/\D/g, '')
  if (digits.length === 4) {
...
export default function PurchaseBill() {
  ...
  const [billNo, setBillNo] = useState('P0001') // Auto-incrementing Purchase Invoice No
  ...
  const handlePartyInvKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' || e.key === 'Tab') {
      ...
      if (e.key === 'Enter') {
        setTimeout(() => invDateRef.current?.focus(), 20)
      }
    }
  }
  ...
  const confirmSaveBill = () => {
    ...
    setTimeout(() => {
      ...
      const numPart = parseInt(billNo.replace(/\D/g, '')) || 1
      setBillNo(`P${String(numPart + 1).padStart(4, '0')}`)
      setPartyName('')
      setPartyInvNo('')
    ...
    <div style={{ backgroundColor: 'var(--color-bg)', flex: 1, display: 'flex', flexDirection: 'column', gap: '3px', overflow: 'hidden', color: 'var(--color-text-primary)', padding: 0 }}>
      ...
      <div style={{ backgroundColor: 'var(--color-bg-surface)', border: '1px solid var(--color-border)', borderRadius: '8px', padding: '5px 10px', boxShadow: 'var(--shadow-sm)', display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'nowrap', overflowX: 'auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', width: '80px', flexShrink: 0 }}>
          <label style={{ fontSize: '10px', fontWeight: '600', color: 'var(--color-text-secondary)', textTransform: 'uppercase' }}>Entry No.</label>
          <input 
            ref={billNoRef} type="text" value={billNo} onChange={e => setBillNo(e.target.value)} onKeyDown={handleBillNoKeyDown}
            style={{ width: '100%', backgroundColor: 'var(--color-bg)', border: '1px solid var(--color-border-strong)', borderRadius: '4px', padding: '3px 6px', fontSize: '12px', color: 'var(--color-primary)', fontWeight: 'bold', outline: 'none' }}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', width: '105px', flexShrink: 0 }}>
          <label style={{ fontSize: '10px', fontWeight: '600', color: 'var(--color-text-secondary)', textTransform: 'uppercase' }}>Party Inv No</label>
          <input 
            ref={partyInvRef} type="text" value={partyInvNo} onChange={e => setPartyInvNo(e.target.value)} onKeyDown={handlePartyInvKeyDown} placeholder="INV-102"
            style={{ width: '100%', backgroundColor: 'var(--color-bg)', border: '1px solid var(--color-border-strong)', borderRadius: '4px', padding: '3px 6px', fontSize: '12px', color: 'var(--color-text-primary)', outline: 'none' }}
            onFocus={e => { e.target.style.borderColor = 'var(--color-primary)'; e.target.select(); }}
            onBlur={e => e.target.style.borderColor = 'var(--color-border-strong)'}
          />
        </div>
                      ...
                      onKeyDown={e => { 
                        ...
                        if (e.key === 'Enter') {
                          if (row.product && (!row.batch || !row.batch.trim())) {
                            e.preventDefault()
                            alert('Batch Number is mandatory! Please enter Batch Number or press F3 to select existing.')
                            return
                          }
                ...
                <input 
                  ...
                  onKeyDown={e => { 
                    ...
                    if (e.key === 'Enter' || e.key === 'Tab') {
                      ...
                      if (!val.trim()) {
                        e.preventDefault()
                        alert('Batch Number is mandatory! Please enter Batch Number or press F3 to select existing batch.')
                        return
                      }
                      if (e.key === 'Enter') {
                        document.getElementById('bottom-expiry-input')?.focus() 
