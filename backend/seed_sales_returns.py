...
def run():
    ...
    try:
        ...
        print(f"Seeding sales returns for organization: {org.name}")
        ...
        for i in range(1, 11):
            ...
            # Select 1 to 3 random batches
            num_items = random.randint(1, min(3, len(available_batches)))
            ...
            for b in selected_batches:
                ...
                # Calculate item taxes based on sale rate
                return_rate = float(b.rate_a) if b.rate_a else float(p.rate_a) if p.rate_a else float(p.mrp)
                ...
                # A sales return INCREASES stock (customer returns goods to our warehouse)
                b.current_stock += qty
            ...
            inv = models.Invoice(
                ...
                invoice_type="sales-return-bill",
                invoice_number=f"SR-{(datetime.utcnow().year)}-{i:04d}",
        ...
        print(f"Successfully seeded 10 sales return invoices and {len(new_items)} items.")
